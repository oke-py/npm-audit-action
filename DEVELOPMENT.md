...
- Before release-please runs, the release workflow applies the `changes-dist`
  label to merged PRs whose completed diff contains a file under `dist/`.
  Release notes are generated by GitHub and include only PRs with that label.
  This keeps build and development dependency updates out of the notes unless
  they change the action bundle that users run.
- `build:` commits are configured to bump the patch version so that a
  development dependency update that changes `dist/` can be released. The
  `changes-dist` label controls whether the PR is shown in the release notes;
  it does not control version calculation.
